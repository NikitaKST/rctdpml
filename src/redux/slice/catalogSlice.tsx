...
export const fetchAddCatalog = createAsyncThunk(
  ...
  async function ({ id, length }: AddProduct, thunkAPI) {
    try {
      ...
      if (id === 11) {
        response = await fetch(
          `http://localhost:7070/api/items?offset=${length}`
        );
      } else {
        response = await fetch(
          `http://localhost:7070/api/items?categoryId=${id}&offset=${length}`
        );
      }
      if (!response.ok) {
        throw new Error('Server Error!');
  ...
  async function (search: string, thunkAPI) {
    try {
      const response = await fetch(
        `http://localhost:7070/api/items?q=${search}`
      );
      if (!response.ok) {
        throw new Error('Server Error!');
      }
...
const catalog = createSlice({
  ...
  extraReducers: builder => {
    ...
    builder.addCase(
      ...
      (state, action: PayloadAction<Array<PropsCategory>>) => {
    ...
    builder.addCase(
      ...
      (state, action: PayloadAction<Array<PropsProduct>>) => {
        (state.status = 'resolver'), (state.cards = action.payload);
      }
    );
    builder.addCase(fetchCatalog.rejected, (state, action) => {
      state.status = 'rejected';

  ...
  return (
    <>
      <section className="cart">
          ...
          {cards.map((card: PropsCartProduct) => (
            <tbody key={`${card.id}${card.size}`}>
              <tr>
                ...
                <td>
                  <a href={`/products/${card.id}`}>{card.title}</a>
                </td>
                <td>{card.size}</td>
                <td>{card.quantity}</td>
        ...
        {status === 'resolver' && (
          <h2 className="text-center">Поздравляем! Заказ успешно оформлен!</h2>
        )}
      </section>
          ...
          <div className="card-order">
            <form onSubmit={makeAnOrder} className="card-body">
              <div className="form-group">
                <label className="phone__form-group" htmlFor="phone">
                  Телефон
                </label>
                <input
                  type="text"
              ...
              <div className="form-group">
                <label className="address__form-group" htmlFor="address">
                  Адрес доставки
                </label>
                <input
                  type="text"
